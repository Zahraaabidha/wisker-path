function init() {
  ...
  const bone = `<div class="bone"></div>
    <svg style="display:none" x="0px" y="0px" width="100%" height="100%" viewBox="0 0 151.9 151.9" >
    <style type="text/css">
    .st0{fill:#FFFFFF;}
    </style>
  ...
  const setX = num =>{
    x = num
    spriteContainer.style.left = `${x - cellSize * 0.3}px`
  }

  const setY = num =>{
    y = num
    spriteContainer.style.top = `${y - cellSize * 0.45}px`
  }

  const setSpriteFrame = (col, row) => {
    spriteFrame = [col, row]
    const xPct = (col / 3) * 100
    const yPct = (row / 7) * 100
    spriteContainer.style.backgroundPosition = `${xPct}% ${yPct}%`
  }

  const nextCol = () => (spriteFrame[0] + 1) % 4
  ...
  const decideNextMove = (current, count) =>{
    ...
    const minValue = Math.min(...mapInfo.map(cell=> cell.distanceToGoal))
    const optionsWithMinValue = mapInfo.filter(cell=> cell.distanceToGoal === minValue)

    mapInfo.filter(cell => cell.distanceToGoal !== minValue).forEach(option=>{
  ...
  const animateHappyCat = () => {
    setSpriteFrame(2, 4)
    happy = setTimeout(() => {
      happy = setInterval(() => {
        setSpriteFrame(nextCol(), 5)
      }, 200)
    }, 10000)
  }
  ...
  // window.addEventListener('keydown', spriteWalk)
   ...
   const resize = () =>{
    ...
    positionSprite(start)
    setSpriteFrame(0, 0)
    spriteContainer.style.height = `${cellSize * 1.6}px`
    spriteContainer.style.width = `${cellSize * 1.6}px`
